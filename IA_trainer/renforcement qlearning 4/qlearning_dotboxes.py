...
REWARD_ENOUGH = 0.80 # gagne + des 3/4 du temps
NB_GAMES_TO_WIN = 40 #nb de parties sur lesquelles prend moyenne du niveau
...
update_after_actions = 1 #ne modifie pas le modèle à chaque coup
# How often to update the target network
update_target_network = 10 #10000 #modifie encore moins souvent le second modele (stabilisteur, chargé d'estimer les récompenses à coup + 2)
...
for game_count in range(NB_ROUND_MAX): #nb jeu max
    ...
    while True: #112 coups / jeu
        ...
        is_player1, lineboard_next, end_game = env.decide_and_move(action)
        ...
        if is_player1 == IS_PLAYER1:
            ...
            # rewards_history.append(reward) #différence se crée ici
            end_game_history.append(end_game)

            # Limit the lineboard and reward history
            if len(board_history) > max_memory_length:
    ...
    reward = 1 if IS_PLAYER1 and results[0] > results[1] else 0

    for i in range(len(end_game_history) - len(rewards_history)):
        rewards_history.append(reward) #différence se récupère ici


    if move_count % update_after_actions == 0: #si assez de données, tous les 4 coups, met à jour modèle 1

        # Get indices of samples for replay buffers
        indices = [x for x in range(len(end_game_history))]#sélectionne échantillon au hasard
    ...
    if game_count % update_target_network == 0: #tous les 10000 coups, met à jour modèle 2
