...
class Agent:
	...
	def __init__(self, load_trained: bool, moves_list):
		...
		self.nn_learningRate = 0.01 #min e^(-3) = 0.05
	...
	def get_probs(self, normalized_lineboard) -> ([float], [float]):
		...
		"""
		q_values = self.model.predict(normalized_lineboard)
	...
	def play_competitive(self, lineboard) -> (int, int): # must be ultra optimized
		...
		probs, qvalues = self.get_probs(self.normalize_nn_input(lineboard))
	...
	def play_train(self, lineboard, moves_remaining) -> (int, int): # an heavier version of play
		...
		self.lineboard_by_turn.append(normalized_lineboard)

		# get model predictions
		probs, qvalues = self.get_probs(normalized_lineboard)
	...
	def train(self, lineboard, ai_score: int, other_score: int):
		...
		if self.train_model:
			...
			for board, target in zip(self.lineboard_by_turn, targets):
				self.model.fit(board, target, verbose=0) #verbose=1, callbacks=[reduce_lr])
