...
class Agent:
	...
	def play(self, lineboard, moves_remaining):
		...
		normalized_lineboard_corrected = normalized_lineboard[0]
		normalized_lineboard_corrected[[index for index in range(len(probs)) if lineboard[index] != OWNER_NONE]] = -999
		...
		if 0.9 * max(normalized_lineboard_corrected) > normalized_lineboard_corrected[move]:
			move = np.argmax(normalized_lineboard)
			self.nb_correction += 1
			print(f"correction {self.nb_correction}")
